...
const Sidebar = () => {
  ...
  const [input , setInput] = useEffect(false);


  const navigate = useNavigate();

  const filteredUsers = input? users.filter((user)=>user.fullName.toLowerCase().includes(input.toLowerCase(input.toLowerCase()))) : users;

  useEffect(()=>{

  },[onlineUsers])
  return (
    <div className={`bg-[#8185B2]/10 h-full p-5 rounded-r-x1 overflow-y-scroll text-white ${selectedUser ? "max-mid:hidden" : ''}`}>
      ...
      <div className='flex flex-col'>
        {filteredUsers.map((user , index)=>(
          <div onClick={()=>{setSelectedUser(user); setUnseenMessages(prev=>({...prev,[user._id]:0}))}} key={index} className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text-sm ${ selectedUser?._id === user._id && 'bg-[#282142]/50'}`}>
            {/* // profile pic stuff please resolve */}
